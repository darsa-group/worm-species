...
class AdultTaxonPlanTests(unittest.TestCase):
    def test_configs_cover_every_observed_adult_combination(self) -> None:
        ...
        definitions = baseline["evaluation"]["data_holdout_controls"][
            "definitions"
        ]
        self.assertEqual(
        ...
        self.assertEqual(planned_holdouts, EXPECTED_HOLDOUTS)
        self.assertTrue(
            all(
                definition["where"]["age"] == "Adult"
                and definition["primary_tasks"] == ["genus", "species", "age"]
...
class AdultTaxonReportTests(unittest.TestCase):
    ...
    def _write_run(
        ...
        *,
        ...
        evaluation_dir = "data_holdout_evaluation"
        if control:
            evaluation_dir = "data_holdout_control_evaluation"
            config["evaluation"] = {
                "data_holdout_controls": {"definitions": [holdout]}
            }
        else:
            config["data_holdout"] = holdout
        ...
        score_base = 0.80 if control else 0.60
        score_base += 0.02 * (seed - 40) + 0.05 * (weight > 0)
        rows = []
        for cohort in ("development_withheld", "independent_test"):
            for task in ("genus", "species", "age"):
                rows.append({
                    ...
                    "cohort": cohort,
                    "task": task,
                    "target_label": holdout["where"].get(
                        "age" if task == "age" else task
                    ),
                    "target_n": 10,
                    "target_recall": score_base,
                    "target_recall_image": score_base,
                    "class_supported_by_training_head": True,
                })
        evaluation = run / evaluation_dir
        evaluation.mkdir()
        pd.DataFrame(rows).to_csv(evaluation / "task_metrics.csv", index=False)

    def test_report_writes_hloss_and_non_hloss_figures_with_sources(self) -> None:
...
class PaperReportRegressionTests(unittest.TestCase):
    def test_incomplete_result_root_generates_manifest_without_crashing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
        ...
        self.assertEqual(summary["best_baseline"], {})
    ...
    def test_original_test_summary_preserves_run_directory(self) -> None:
        ...
        self.assertEqual(summary["percent"].tolist(), [50])
