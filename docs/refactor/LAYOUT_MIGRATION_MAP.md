...
| cache, sweep, colour, cue, and node-local shell launchers | `legacy/slurm/` | `scripts/slurm/` and root symbolic links preserve the exact shell content |
...
| --- | --- | --- |
| `config_old.yaml` | `legacy/configs/config_old.yaml` | `configs/config_old.yaml` and root symbolic links |
| `src/generate_sweep_run_specs.py` | `legacy/python/experiments/generate_sweep_run_specs.py` | executable Python wrapper preserves imports, CLI help, and generated bytes |
