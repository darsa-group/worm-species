...
def _array_context(
    ...
    *,
    ...
    scratch_root = str(scratch.get("root", "/tmp/worm_species"))
    if node_local:
        ...
        runtime_metadata = f"{runtime_data}/01_Segmented/global_metadata.csv"
    else:
...
def _build_jobs(
    ...
    if bool(wandb_config.get("enabled", False)):
        ...
        train_job["exports"].update(
            {
                "WANDB_ENABLED": "true",
                "WANDB_PROJECT": str(wandb_config.get("project") or "worm-species"),
